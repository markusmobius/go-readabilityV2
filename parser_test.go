...
func TestLibraryBoundary(t *testing.T) {
	err := fp.WalkDir(".", func(path string, entry os.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		if entry.IsDir() {
			if path != "." && (strings.HasPrefix(entry.Name(), ".") || entry.Name() == "test-pages" || entry.Name() == "target") {
				return fp.SkipDir
			}
			return nil
		}
		...
		source, err := goparser.ParseFile(token.NewFileSet(), path, nil, 0)
		if err != nil {
			return err
		}
		if source.Name.Name == "main" {
			t.Errorf("application package in extraction library: %s", path)
		}
		for _, imported := range source.Imports {
			...
			switch name {
			case "net", "net/http", "log", "log/slog", "os/exec":
				t.Errorf("non-core import %q in %s", name, path)
			}
		}
		ast.Inspect(source, func(node ast.Node) bool {
			switch declaration := node.(type) {
			case *ast.FuncDecl:
				if declaration.Name.Name == "FromURL" {
					t.Errorf("URL fetching is not a library entry point: %s", path)
				}
			case *ast.TypeSpec:
				if declaration.Name.Name == "RequestWith" {
					t.Errorf("HTTP request configuration is not a library API: %s", path)
				}
			}
			return true
		})
